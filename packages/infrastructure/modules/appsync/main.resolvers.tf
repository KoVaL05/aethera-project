resource "aws_appsync_resolver" "unit_kind_resolvers" {
  ...
  field       = each.key
  code        = file(format("%s/%s/index.js", path.root, each.value.path))
...
resource "aws_appsync_resolver" "private_unit_kind_resolvers" {
  for_each = {
    for key, value in local.private_resolvers : key => value
    if value.kind == "UNIT"
  }
